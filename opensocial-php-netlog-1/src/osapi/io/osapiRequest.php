...
class osapiRequest {
  ...
  public static function createRequest($method, $params) {
    ...
    if (self::getService($method) != 'cache' && self::getService($method) != 'system') {
	    ...
	    if (isset($params['groupId']) && ! in_array($params['groupId'], array('@self', '@all', '@friends', '@friendsoffriends', '@supportedMood'))) {
	      throw new osapiException("Invalid groupId:" .$params['groupId'] . " allowed types are: @self, @friendsoffriends, @all and @friends");
	    }
    }
